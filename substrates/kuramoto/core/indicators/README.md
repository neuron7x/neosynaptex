...
The `core/indicators` module provides geometric and phase-synchrony indicators for algorithmic trading signal generation, including Kuramoto order-parameter analysis, Ricci-flow curvature detection, multi-scale coherence analysis, and entropy-based market regime classification.
...
- Provide filesystem-backed caching and feature persistence.
- Target sub-100 ms latency for real-time signal generation (target, not measured here).
...
| --------- | ---- | -------- | ----------- |
| `KuramotoIndicator` | Class | `trading.py` | Kuramoto order parameter calculator with configurable coupling strength |
